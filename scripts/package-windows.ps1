...
try {
    ...
    $packageVersion = ($metadata.packages | Where-Object name -EQ 'steamcounter').version
    if ($packageVersion -notmatch '^(\d+)\.(\d+)\.0(?:-([a-z]))?$') {
        throw 'SteamCounter versions must use X.Y.0 or X.Y.0-letter internally'
    }
    $releaseVersion = "$($Matches[1]).$($Matches[2])$($Matches[3])"
    $output = Join-Path $repoRoot "target/packages/SteamCounter-$releaseVersion-windows-x64.exe"
